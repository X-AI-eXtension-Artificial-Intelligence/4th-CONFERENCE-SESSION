...
def apply_mask_on_image(image, mask, color=(0, 255, 0), alpha=0.5):
    """
    ...
    # Making sure that the mask has the same height and width as the original image
    if mask.shape[:2] != image.shape[:2]:
        mask = cv2.resize(mask, (image.shape[1], image.shape[0]))
        
    # Create an image of the same shape but consists of the specified color
    img_mask = np.zeros_like(image)
    img_mask[:, :, :] = color

    # Add the mask to the image
    img_with_mask = cv2.addWeighted(image, 1, img_mask, alpha, 0)
    
    # Applying the binary mask
    img_with_mask[mask==0] = image[mask==0]
    
    return img_with_mask



def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None, masks=None):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        if masks is not None:
            for mask in masks:
                im = mask_cv2(im, mask[0], random_color=True)
    return im

def mask_cv2(image, mask, random_color=False):
    if random_color:
        color = np.random.randint(0, 256, size=3)  # BGR
    else:
        color = np.array([255, 144, 30])  # BGR for DeepSkyBlue color
    ...
    blended = cv2.addWeighted(image, 1, mask_3ch, 0.6, 0)
